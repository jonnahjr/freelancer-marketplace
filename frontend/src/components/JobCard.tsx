    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right ml-4">
          ...
          <div className="font-bold text-gray-900 dark:text-white">{job.amount ? `$${job.amount}` : 'TBD'}</div>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="flex gap-2 flex-wrap">
          {(job.skills || ['React', 'Tailwind']).slice(0, 4).map((s: string, i: number) => (
            <span key={i} className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-700 rounded border text-gray-600 dark:text-gray-300">{s}</span>
          ))}
        </div>
